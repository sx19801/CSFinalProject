...
class PredPreyEnv(gym.Env):
    ...
    def __init__(self, render_mode="human", grid_height=GRIDSIZE, grid_width=GRIDSIZE, num_prey=2, num_pred=1, num_berry=0 , num_channels=4):
    ...
    def _get_obs(self, prey_locations):      #adds all the stuff to the grid
        ...
        for location in self.pred_locations:
            ...
            for dx,dy in [(-1, 0),(1,0),(0,1),(0,-1),(0,0)]:  # Offset in the x direction
    ...
    def step(self, actions):
        ...
        for index, scalar_action in enumerate(self.scalar_actions):
            ...
            if scalar_action != -1:        #dead check
                ...
                elif scalar_action == 4: #SHOUT
                    print(f"AHHHHH LOOK AT ME.... says prey {index}")
                    new_prey_locations[index] = self.prey_locations[index]
                    if self.prey_shouting_this_turn == False:
                        self.target_prey = index
                        self.prey_shouting_this_turn = True
                    elif self.prey_shouting_this_turn:      #if multiple people shouting then random choice
        ...
        if self.turns_until_move_counter == self.turns_until_move:
            self.move_pred()
            #print(f"self.currently pursuing locations after pred move: {self.currently_pursuing_locations}")
            print(f"self.currently pursuing after pred move: {self.currently_pursuing}")
            self.turns_until_move_counter = 0
        ...
        for i in range(self.num_prey):
            if self.active_prey[i] == True:
                ...
                for j in range(self.num_pred):
                    ...
                    # Loop through all cells within one square of the predator
                    for dx, dy in [(-1, 0),(1,0),(0,-1),(0,1)]:  # Offset in the x direction
                    # Offset in the y direction
                        # Calculate the potentially dangerous cell's coordinates
                        ...
                        if np.array_equal(new_prey_locations_resolved[i], [danger_zone_x, danger_zone_y]):
                            ...
                            self.prey_died_this_turn = True
                            if self.target_prey == i:
                                self.target_prey = None
        ...
        if self.prey_died_this_turn == False and self.prey_shouting_this_turn:
            print("ALTRUISM POSSIBLY")
